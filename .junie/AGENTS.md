...
#### 12. Use Testcontainers for integration tests (PostgreSQL)

- Prefer Testcontainers for integration tests to ensure high fidelity with the production PostgreSQL database.
- Configure a dedicated Spring profile (e.g., `testcontainers`) that dynamically provisions a PostgreSQL instance via Testcontainers.
- Keep migrations enabled in tests so the schema is created with Flyway before tests run, validating migrations against a real PostgreSQL server.
- Use `@Transactional` in integration tests to ensure data changes are rolled back after each test, providing a clean slate.

**Explanation:**

- Testcontainers provide a real PostgreSQL instance, mirroring the production environment. This significantly reduces the risk of environment-specific bugs or SQL compatibility issues that might be missed when testing against an in-memory database like H2.
- While Testcontainers might introduce a slightly longer startup time compared to H2, the benefit of testing against a true database environment often outweighs this, particularly for projects with complex database interactions or vendor-specific SQL.
- Running Flyway against a real PostgreSQL server validates your migrations and ensures your schema is correctly built for test execution, exactly as it would be in production.
- `@Transactional` annotation on test methods or classes ensures that all data modifications performed during a test are automatically rolled back upon completion, guaranteeing isolated and repeatable tests.

**Configuration:**

- Add the `org.testcontainers:postgresql` and `org.testcontainers:junit-jupiter` dependencies to your `pom.xml`.
- Create `application-testcontainers.yml` (or `.properties`) to configure the datasource for tests using Testcontainers. This typically involves using a `DynamicPropertySource` in your test setup or Spring Boot's automatic Testcontainers detection.
- Annotate integration tests with `@ActiveProfiles("testcontainers")` and `@Transactional`.
...
* Use PostgreSQL compliant SQL syntax for database migrations.
...
### 28. Simplicity First: No Parallelism or Multithreading

We intentionally avoid multithreading and parallel execution to keep the code simple, predictable, and easier to maintain and test.

- Do not introduce threads, executors, or background pools.
- Do not introduce asynchronous or reactive execution paths that execute on multiple threads.
- Prefer straightforward, sequential control flow.

#### What this means in practice

- Java and Spring:
    - Do not use parallelStream(), CompletableFuture, Thread, ExecutorService, @Async, WebFlux parallel() or boundedElastic/parallel schedulers, or Reactor subscribeOn/publishOn pointing to non-single schedulers.
    - Keep @Scheduled jobs strictly single-threaded. Allow only one run at a time and avoid overlapping execution.
    - Do not rely on Atomic* types or locks unless there is a proven need. Use simple local variables in single-threaded flows.

- Persistence and transactions:
    - Keep transactional work inside a single thread. No parallel DB calls within a single unit of work.
    - No fan-out parallel queries or writes; serialize the work.

- Tests and builds:
    - Do not run tests in parallel. Keep test execution single-threaded for determinism and simpler debugging.
    - Do not configure Maven/Failsafe/Surefire to run modules or tests in parallel.

- Frontend:
    - Avoid Web Workers or parallel computation patterns for application logic.
    - Prefer sequential Promise chains over Promise.all for CPU-heavy workflows that could contend for resources.

#### Allowed patterns

- Regular Java streams (stream(), not parallelStream()).
- Sequential @Scheduled jobs with a single-threaded scheduler.
- Synchronous MVC stack (Spring MVC) rather than WebFlux.
- Plain synchronous service methods.

#### Spring Boot configuration (explicit single thread)

If you need to make the single-thread scheduling explicit, add a TaskScheduler bean with a pool size of 1:
...
public TaskScheduler taskScheduler() {
    ...
    scheduler.setPoolSize(1); // force single-threaded scheduling
...
Do not define any additional TaskExecutor/Executor beans for background work. Avoid @Async entirely.

#### @Scheduled jobs: avoid overlaps

- With a single-threaded TaskScheduler, triggers are queued and will not overlap in parallel.
- Prefer fixedDelay over cron if you want to ensure every run starts only after the previous run completes:

```java
// Example: run 5 minutes after the previous run finished
@Scheduled(fixedDelayString = "${app.scheduling.fixed-delay}")
void run() { ... }
```

If you must use cron, keep the interval larger than the longest expected execution time so the next trigger won’t arrive before the previous run ends.

#### JUnit and Maven: keep tests single-threaded

- JUnit 5 property (src/test/resources/junit-platform.properties):

```
junit.jupiter.execution.parallel.enabled=false
```

- Maven Surefire/Failsafe (pom.xml): do not enable any parallel settings. If present, remove or set to none:
...
#### Code-level migration tips

- Replace parallelStream() with stream():
    - Before: list.parallelStream().map(...).toList();
    - After:  list.stream().map(...).toList();

- Remove Atomic* counters in single-threaded flows:
    - Before: AtomicInteger count = new AtomicInteger(); count.incrementAndGet();
    - After:  int count = 0; count++;

- Remove manual concurrency guards when single-threaded scheduling is guaranteed:
    - If a single-thread scheduler is used, an AtomicBoolean “running” guard is typically unnecessary.

- Avoid CompletableFuture.supplyAsync(...) or any explicit Executor usage; call methods directly.

#### Rationale

- Predictability: sequential execution eliminates race conditions and timing bugs.
- Simpler testing: deterministic single-thread behavior simplifies unit and integration testing.
- Operational clarity: easier to reason about performance and failure modes without concurrency.

---

### Optional, project-specific notes

- PDF2ZUGFeRD: Ensure that all conversions are correctly logged in the PostgreSQL database and that the processing status is updated according to the result.
- Ensure there are no @Async annotations or Executor beans. Prefer synchronous service calls end-to-end.
